...
enum clock_osc_freq clock_get_osc_freq(void)
{
	...
	switch (reg) {
	case 0x5:
		return CLOCK_OSC_FREQ_38_4;

	case 0x8:
		return CLOCK_OSC_FREQ_12_0;
...
static void tegra210_set_up_pllp(void)
{
	struct clk_rst_ctlr *clkrst = (struct clk_rst_ctlr *)NV_PA_CLK_RST_BASE;
	u32 value;

	/* Set PLLP_OUT1, 3 & 4 frequencies to 9.6, 102 & 204 MHz */

	/* OUT1 */
	/* Assert RSTN before enable */
	value = PLLP_OUT1_RSTN_EN;
	writel(value, &clkrst->crc_pll[CLOCK_ID_PERIPH].pll_out[0]);

	/* Set divisor and reenable */
	value = (IN_408_OUT_9_6_DIVISOR << PLLP_OUT1_RATIO) |
		PLLP_OUT1_OVR | PLLP_OUT1_CLKEN | PLLP_OUT1_RSTN_DIS;
	writel(value, &clkrst->crc_pll[CLOCK_ID_PERIPH].pll_out[0]);

	/* OUT3 and OUT4 */
	/* Assert RSTN before enable */
	value = PLLP_OUT4_RSTN_EN | PLLP_OUT3_RSTN_EN;
	writel(value, &clkrst->crc_pll[CLOCK_ID_PERIPH].pll_out[1]);

	/* Set divisor and reenable */
	value = (IN_408_OUT_204_DIVISOR << PLLP_OUT4_RATIO) |
...
unsigned clk_m_get_rate(unsigned parent_rate)
{
	struct clk_rst_ctlr *clkrst =
		(struct clk_rst_ctlr *)NV_PA_CLK_RST_BASE;
	u32 value, div;
...
#define PLLE_MISC1 0x6c
#define  PLLE_MISC1_SDM_RESET (1 << 4)
#define  PLLE_MISC1_EN_DITHER (1 << 2)
#define  PLLE_MISC1_EN_SSC (1 << 1)
#define  PLLE_MISC1_EN_SDM (1 << 0)

#define PLLE_BASE 0x0e8
#define  PLLE_BASE_ENABLE (1 << 30)
#define  PLLE_BASE_LOCK_OVERRIDE (1 << 29)
#define  PLLE_BASE_PLDIV_CML_MASK (0xf << 24)
#define  PLLE_BASE_PLDIV_CML(x) (((x) & 0xf) << 24)
#define  PLLE_BASE_NDIV_MASK (0xff << 8)
#define  PLLE_BASE_NDIV(x) (((x) & 0xff) << 8)
#define  PLLE_BASE_MDIV_MASK (0xff << 0)
...
#define  PLLE_SS_CNTL1_SDM_DIN(x) (((x) & 0xffff) << 16)
...
static void tegra_plle_disable(void)
{
	u32 value;
...
int tegra_plle_enable(void)
{
	...
	value = readl(NV_PA_CLK_RST_BASE + PLLE_MISC1);
	value &= ~PLLE_MISC1_SDM_RESET;
	value |= PLLE_MISC1_EN_DITHER;
	value &= ~PLLE_MISC1_EN_SSC;
	writel(value, NV_PA_CLK_RST_BASE + PLLE_MISC1);

	value = readl(NV_PA_CLK_RST_BASE + PLLE_SS_CNTL);
	value |= PLLE_SS_CNTL_SSCBYP | PLLE_SS_CNTL_BYPASS_SS;
	writel(value, NV_PA_CLK_RST_BASE + PLLE_SS_CNTL);

	value = readl(NV_PA_CLK_RST_BASE + PLLE_MISC1);
	value &= ~PLLE_MISC1_EN_SDM;
	writel(value, NV_PA_CLK_RST_BASE + PLLE_MISC1);

	value = readl(NV_PA_CLK_RST_BASE + PLLE_SS_CNTL1);
	value |= PLLE_SS_CNTL1_SDM_DIN(0);
	writel(value, NV_PA_CLK_RST_BASE + PLLE_SS_CNTL1);

	value = readl(NV_PA_CLK_RST_BASE + PLLE_BASE);
	value &= ~PLLE_BASE_MDIV_MASK;
	value |= PLLE_BASE_MDIV(2);
	value &= ~PLLE_BASE_NDIV_MASK;
	value |= PLLE_BASE_NDIV(124);
	value &= ~PLLE_BASE_PLDIV_CML_MASK;
	value |= PLLE_BASE_PLDIV_CML(14);
	writel(value, NV_PA_CLK_RST_BASE + PLLE_BASE);
